...
Legd_ang1_west = [110:1:222];                                           % L1 이동경로설정(WEST)
...
Legd_ang2_west = [110:1:222];                                           % L2 이동경로 설정(WEST)
...
Legd_ang1_north = [430:-1:374,linspace(374,374,84)];                    % L1 이동경로설정(NORTH)
...
Legd_ang2_north = [430:-1:374,linspace(374,374,84)];                    % L2 이동경로 설정(NORTH)
...
Legd_ang1_south = [110:1:166,linspace(166,166,84)];                     % L1 이동경로설정(SOUTH)
...
Legd_ang2_south = [110:1:166,linspace(166,166,84)];                     % L2 이동경로 설정(SOUTH)
...
Legu_r1_east = 13.5;                                                       % L1 원의반지름
Legu_x1_east = Legu_r1_east*cos(Legu_theta1_east)+11.5;                % X축 평행이동 및 설정
...
Legu_r2_east = 13.5;                                                       % L2 원의반지름
Legu_x2_east = Legu_r2_east*cos(Legu_theta2_east)+11.5;                % X축 평행이동 및 설정
Legu_z2_east = Legu_r2_east*sin(Legu_theta2_east)-16;                   % Z축 평행이동 및 설정

Legu_ang1_west = [250:-1:110];                                          % L1 이동경로설정(WEST)
Legu_theta1_west = Legu_ang1_west*(pi/180);
Legu_r1_west = 13.5;                                                       % L1 원의반지름
Legu_x1_west = Legu_r1_west*cos(Legu_theta1_west)-11.5;                % X축 평행이동 및 설정 
Legu_z1_west = Legu_r1_west*sin(Legu_theta1_west);                      % Z축 설정

Legu_ang2_west = [250:-1:110];                                          % L2 이동경로 설정(WEST)
Legu_theta2_west = Legu_ang2_west*(pi/180);
Legu_r2_west = 13.5;                                                       % L2원의반지름
Legu_x2_west = Legu_r2_west*cos(Legu_theta2_west)-11.5;                % X축 평행이동 및 설정
Legu_z2_west = Legu_r2_west*sin(Legu_theta2_west)-16;                   % Z축 평행이동 및 설정

Legu_ang1_north = [356:1:430,linspace(430,430,66)];                     % L1 이동경로설정(NORTH)
Legu_theta1_north = Legu_ang1_north*(pi/180);
Legu_r1_north = 13.5;                                                      % 원의반지름
Legu_y1_north = Legu_r1_north*cos(Legu_theta1_north)+11.5;             % Y축 평행이동 및 설정 
Legu_z1_north = Legu_r1_north*sin(Legu_theta1_north);                   % Z축 설정

Legu_ang2_north = [356:1:430,linspace(430,430,66)];                     % L2 이동경로 설정(NORTH)
Legu_theta2_north = Legu_ang2_north*(pi/180);
Legu_r2_north = 13.5;                                                      % 원의반지름
Legu_y2_north = Legu_r2_north*cos(Legu_theta2_north)+11.5;             % Y축 평행이동 및 설정
Legu_z2_north = Legu_r2_north*sin(Legu_theta2_north)-16;                % Z축 평행이동 및 설정

Legu_ang1_south = [184:-1:110,linspace(110,110,66)];                    % L1 이동경로설정(SOUTH)
Legu_theta1_south = Legu_ang1_south*(pi/180);
Legu_r1_south = 13.5;                                                      % 원의반지름
Legu_y1_south = Legu_r1_south*cos(Legu_theta1_south)-11.5;             % Y축 평행이동 및 설정 
Legu_z1_south = Legu_r1_south*sin(Legu_theta1_south);                   % Z축 설정

Legu_ang2_south = [184:-1:110,linspace(110,110,66)];                    % L2 이동경로 설정(SOUTH)
Legu_theta2_south = Legu_ang2_south*(pi/180);
Legu_r2_south = 13.5;                                                      % 원의반지름
Legu_y2_south = Legu_r2_south*cos(Legu_theta2_south)-11.5;             % Y축 평행이동 및 설정
...
T_r1_south = 13.5;                                                        % 원의반지름
T_y1_south = T_r1_south*cos(T_theta1_south)-11.5;                     % Y축 평행이동 및 설정 
T_z1_south = T_r1_south*sin(T_theta1_south);                           % Z축 설정

T_ang2_south = 110;                                                    % L2 이동경로 설정(SOUTH)
T_theta2_south = T_ang2_south*(pi/180);
T_r2_south = 13.5;                                                        % 원의반지름
T_y2_south = T_r2_south*cos(T_theta2_south)-11.5;                     % Y축 평행이동 및 설정
...
ha2_vertex_surface=[-40 -40 10.15 ; 40 -40 -36.1 ; 40 40 -36.1 ; -40 40 10.15];                              % 평면 만들기!!
...
move_surface_up = linspace(-36.1,10.15,length(aL));
move_surface_down = linspace(10.15,-36.1,length(aL));
...
move_surface_up1 = linspace(-36.1,-19,length(aL)+1);
move_surface_down1 = linspace(11.15,-6,length(aL)+1);
...
move_surface_up2 = linspace(-36.1,5,300);
move_surface_down2 = linspace(11.15,-31,length(move_surface_up2));

g_z2_east =linspace(-2,-20,length(move_surface_up2));
g_x2_east = linspace(4.86,4.86,length(g_z2_east));
...
% g_z2_west =-25:0.1:0; 에서 g_z2_west =-25:0.1:-3; 으로 변경해야하고 이에 따라 다른 값도
% 바꿔줘야함!!
g_z2_west =linspace(-25,-3,length(move_surface_down2)); 
g_x2_west = linspace(-10.3,-10.3,length(g_z2_west));
...
move_surface_up2 = linspace(5,-37,200);
move_surface_down2 = linspace(-31,10.5,length(move_surface_up2));

g_z2_east =linspace(-20,-3.3,length(move_surface_up2));
g_x2_east = linspace(4.86,4.86,length(g_z2_east));
...
g_z2_west =linspace(-3,-26,length(move_surface_down2)); 
g_x2_west = linspace(-10.3,-10.3,length(g_z2_west));
...
move_surface_up2 = linspace(-37,-12.7,200);
move_surface_down2 = linspace(10.5,-12.7,length(move_surface_up2));

g_z2_east =linspace(-3.3,-12.7,length(move_surface_up2));
g_x2_east = linspace(4.86,4.86,length(g_z2_east));
...
g_z2_west =linspace(-26,-12.7,length(move_surface_down2)); 
g_x2_west = linspace(-10.3,-10.3,length(g_z2_west));
...
move_surface_up2 = linspace(-12.7,6,300);
move_surface_down2 = linspace(-12.7,-31,length(move_surface_up2));

g_z2_north =linspace(-12.7,-1,length(move_surface_up2));
g_y2_north = linspace(13.35,13.35,length(g_z2_north));
...
g_z2_south =linspace(-12.7,-24,length(move_surface_down2)); 
g_y2_south = linspace(-13.35,-13.35,length(g_z2_south));
...
move_surface_up2 = linspace(6,-31,200);
move_surface_down2 = linspace(-31,6,length(move_surface_up2));

g_z2_north =linspace(-1,-24,length(move_surface_up2));
g_y2_north = linspace(13.35,13.35,length(g_z2_north));
...
g_z2_south =linspace(-24,-1,length(move_surface_down2)); 
g_y2_south = linspace(-13.35,-13.35,length(g_z2_south));
...
move_surface_down2 = linspace(6,-12.7,300);
move_surface_up2 = linspace(-31,-12.7,length(move_surface_down2));

g_z2_north =linspace(-24,-12.7,length(move_surface_up2));
g_y2_north = linspace(13.35,13.35,length(g_z2_north));
...
g_z2_south =linspace(-1,-12.7,length(move_surface_down2)); 
g_y2_south = linspace(-13.35,-13.35,length(g_z2_south));
